...
public interface OrderRepository extends JpaRepository<Order, Long> {
        ...
        @Query("SELECT o FROM Order o JOIN FETCH o.customer c LEFT JOIN FETCH o.payment p " +
                "WHERE (:city IS NULL OR c.address.city = :city) " +
                "AND (:street IS NULL OR c.address.street = :street) " +
                "AND (:zipcode IS NULL OR c.address.zipcode = :zipcode)")
        List<Order> findOrdersByAddress(@Param("city") String city, @Param("street") String street, @Param("zipcode") String zipcode);

        @Query("SELECT o FROM Order o JOIN FETCH o.customer c LEFT JOIN FETCH o.payment p " +
                "WHERE (:paymentStatus IS NULL OR " +
                "(:paymentStatus = 'PAID' AND p.amount > 0) OR " +
                "(:paymentStatus = 'PENDING' AND (p IS NULL OR p.amount = 0)))")
